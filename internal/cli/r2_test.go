...
func TestR2BucketCommandsSendExpectedRequests(t *testing.T) {
	...
	stdout, _, err := runR2CLI(t, srv.URL, "r2", "bucket", "list", "--name-contains", "photo")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err := runR2CLI(t, srv.URL, "r2", "bucket", "create", "photos", "--location", "WnAm"); err != nil {
		t.Fatal(err)
	}
	if _, _, err := runR2CLI(t, srv.URL, "--output", "json", "r2", "bucket", "info", "photos"); err != nil {
		t.Fatal(err)
	}
	if _, _, err := runR2CLI(t, srv.URL, "r2", "bucket", "delete", "photos", "--force"); err != nil {
		t.Fatal(err)
	}
...
func TestR2ListFollowsCursorAcrossPages(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch cursor {
		case "":
			_, _ = w.Write([]byte(`{"success":true,"result":{"buckets":[{"name":"first"}]},"result_info":{"cursor":"next-page"}}`))
		case "next-page":
	...
	stdout, _, err := runR2CLI(t, srv.URL, "--output", "json", "r2", "bucket", "list")
	if err != nil {
		t.Fatal(err)
	}
...
func TestR2BucketCommandsDryRun(t *testing.T) {
	cases := []struct {
		...
		{"list", []string{"r2", "bucket", "list", "--name-contains", "photo", "--dry-run"}, "GET", "/r2/buckets", "name_contains=photo&per_page=100", ""},
		{"create", []string{"r2", "bucket", "create", "photos", "--dry-run"}, "POST", "/r2/buckets", "", `{"name":"photos"}`},
		{"info", []string{"r2", "bucket", "info", "photos", "--dry-run"}, "GET", "/r2/buckets/photos", "", ""},
		{"delete", []string{"r2", "bucket", "delete", "photos", "--dry-run"}, "DELETE", "/r2/buckets/photos", "", ""},
	}
...
func TestR2ValidationAndArguments(t *testing.T) {
	...
	for _, bucketName := range []string{"ab", strings.Repeat("a", 65)} {
		for _, args := range [][]string{
			{"r2", "bucket", "create", bucketName, "--dry-run"},
			{"r2", "bucket", "info", bucketName, "--dry-run"},
			{"r2", "bucket", "delete", bucketName, "--dry-run"},
		} {
			_, _, err := runR2CLI(t, "http://example.invalid", args...)
			if err == nil || !strings.Contains(err.Error(), "between 3 and 64") {
				t.Fatalf("expected bucket-length error for %v, got %v", args, err)
			}
		}
	}
	for _, bucketName := range []string{"abc", strings.Repeat("a", 64)} {
		for _, args := range [][]string{
			{"r2", "bucket", "create", bucketName, "--dry-run"},
			{"r2", "bucket", "info", bucketName, "--dry-run"},
			{"r2", "bucket", "delete", bucketName, "--dry-run"},
		} {
			if _, _, err := runR2CLI(t, "http://example.invalid", args...); err != nil {
				t.Fatalf("expected valid boundary name for %v: %v", args, err)
			}
		}
	}
	for _, args := range [][]string{
		{"r2", "bucket", "list", "extra", "--dry-run"},
		{"r2", "bucket", "create", "one", "two", "--dry-run"},
		{"r2", "bucket", "info", "one", "two", "--dry-run"},
		{"r2", "bucket", "delete", "one", "two", "--dry-run"},
	} {
		_, _, err := runR2CLI(t, "http://example.invalid", args...)
		if err == nil {
...
func TestR2HelpIncludesExamples(t *testing.T) {
	for _, args := range [][]string{
		{"r2", "bucket", "list", "--help"},
		{"r2", "bucket", "create", "--help"},
		{"r2", "bucket", "info", "--help"},
		{"r2", "bucket", "delete", "--help"},
	} {
		root := NewRootCmd()
...
func TestR2BucketParentContainsBucketCommands(t *testing.T) {
	root := NewRootCmd()
	var out bytes.Buffer
	root.SetOut(&out)
	root.SetArgs([]string{"r2", "bucket", "--help"})
	if err := root.Execute(); err != nil {
		t.Fatal(err)
	}
	for _, command := range []string{"list", "create", "info", "delete"} {
		if !strings.Contains(out.String(), command) {
			t.Errorf("bucket help missing %q:\n%s", command, out.String())
		}
	}
}
