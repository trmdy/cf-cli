...
// isTunnelUUID reports whether s has the shape of a Cloudflare Tunnel UUID,
// with or without dashes. Tunnel, route, and virtual network IDs all use it.
...
// validateTunnelUUID rejects malformed IDs before a request goes out. An
// empty value is treated as "not given" and left to the caller.
func validateTunnelUUID(label, value string) error {
	if value == "" || isTunnelUUID(value) {
		return nil
	}
	return fmt.Errorf("invalid %s %q: expected a UUID", label, value)
}
...
Examples:
  ...
  cf tunnel list --name prod-tunnel --include-deleted`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return output.RenderTable(cmd.OutOrStdout(), []string{"ID", "NAME", "STATUS", "CREATED"}, rows)
		},
	}
...
Examples:
  ...
  cf tunnel create prod-tunnel --secret "$(openssl rand -base64 32)"`,
	...
	cmd.Flags().StringVar(&secret, "secret", "", "base64-encoded tunnel secret, at least 32 bytes (default: generated by Cloudflare)")
...
func buildTunnelCreateBody(name, secret, configSrc string) ([]byte, error) {
	...
	if secret != "" {
		...
		if len(decoded) < 32 {
			return nil, fmt.Errorf("--secret must decode to at least 32 bytes, got %d", len(decoded))
		}
...
Examples:
  ...
  cf tunnel route list --virtual-network-id 5f2c1a80-1c9e-4f77-8b4d-2f8a37f3b0d1`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := validateTunnelUUID("--virtual-network-id", virtualNetwork); err != nil {
				return err
			}
			client, cfg, err := g.client(true)
			if err != nil {
			...
			q, err := buildTunnelRouteListQuery(tunnelID, virtualNetwork, includeDeleted)
			if err != nil {
				return err
			}
			req := api.Request{Method: "GET", Path: tunnelRoutesPath(accountID), Query: q}
			if g.DryRun {
				dump, err := client.Dump(req)
				if err != nil {
					return err
...
func buildTunnelRouteListQuery(tunnelID, virtualNetwork string, includeDeleted bool) (url.Values, error) {
	if err := validateTunnelUUID("--virtual-network-id", virtualNetwork); err != nil {
		return nil, err
	}
	q := url.Values{}
...
Examples:
  ...
  cf tunnel route add 192.168.4.0/24 --tunnel prod-tunnel --comment "office lan"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Every local flag is checked before the tunnel name lookup, so a
			// bad flag never costs a request.
			if err := validateTunnelRouteOptions(comment, virtualNetwork); err != nil {
				return err
			}
			client, cfg, err := g.client(true)
			if err != nil {
	...
	cmd.Flags().StringVar(&comment, "comment", "", "route comment (at most 100 characters)")
...
// tunnelCommentMaxLen is the API's maxLength for a route comment.
const tunnelCommentMaxLen = 100

// validateTunnelRouteOptions checks the route flags that need no network
// access. Commands call it before resolving a tunnel name so a local mistake
// never costs a request; buildTunnelRouteBody repeats it as the last line of
// defence for callers that build a body directly.
func validateTunnelRouteOptions(comment, virtualNetwork string) error {
	if n := utf8.RuneCountInString(comment); n > tunnelCommentMaxLen {
		return fmt.Errorf("--comment must be at most %d characters, got %d", tunnelCommentMaxLen, n)
	}
	return validateTunnelUUID("--virtual-network-id", virtualNetwork)
}
...
func resolveTunnelRouteID(ctx context.Context, client *api.Client, accountID, ref, virtualNetwork string) (string, error) {
	...
	if !strings.Contains(ref, "/") {
		if !isTunnelUUID(ref) {
			return "", fmt.Errorf("invalid route %q: expected a route ID (UUID) or a network in CIDR notation", ref)
		}
		return ref, nil
	}
	...
	q, err := buildTunnelRouteListQuery("", virtualNetwork, false)
	if err != nil {
		return "", err
	}
