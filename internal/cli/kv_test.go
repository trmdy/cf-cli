...
func TestKVKeyListInvalidLimit(t *testing.T) {
	_, _, err := runKVCLI(t, "http://example.invalid",
		"kv", "key", "list",
		"--namespace", kvTestNamespaceID,
		"--limit", "0",
...
func TestKVKeyGetLive(t *testing.T) {
	var gotPath string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		w.Header().Set("Content-Type", "application/json")
		// Raw text value — client wraps non-envelope responses.
		w.WriteHeader(200)
...
func TestKVHelpIncludesExamples(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			want: []string{"--value", "--namespace", "cf kv key put", "--ttl"},
		},
		{
			args: []string{"kv", "key", "bulk-put", "--help"},
			want: []string{"@file", "cf kv key bulk-put", "--namespace"},
		},
		{
