...
func newPagesProjectCreateCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create <name>",
		Short: "Create a Direct Upload Pages project",
		Long: `Create a Direct Upload Pages project.

This command does not connect a Git repository — that needs a repository
authorization the CLI does not perform — so the project is created for direct
uploads. Build settings given here are stored on the project.
...
Examples:
  ...
  cf pages project create my-site --build-command "npm run build" --destination-dir dist`,
	...
	cmd.Flags().StringVar(&productionBranch, "production-branch", "main", "branch whose deployments are production (required)")
	cmd.Flags().StringVar(&buildCommand, "build-command", "", "command used to build the project")
...
// build_config object when no build flag was given. production_branch is
// required by the API, so an explicitly empty --production-branch is rejected
// here instead of being dropped from the body and failing server-side.
func buildPagesProjectBody(name, productionBranch, buildCommand, destinationDir, rootDir string) ([]byte, error) {
	...
	if branch == "" {
		return nil, errors.New("--production-branch cannot be empty: pass the branch that holds production deployments (for example --production-branch main)")
	}
...
func newPagesDeploymentRollbackCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Roll a Pages project back to an earlier production deployment",
		Long:  "Roll a Pages project back to an earlier production deployment, making it live again.\n\nThe API only rolls back to a successful production deployment: preview\ndeployments and failed builds are rejected.\n\nExamples:\n\n  cf pages deployment rollback my-site f64788e9-fccd-4d4a-a28a-cb84f88f6c8b\n  cf pages deployment rollback my-site f64788e9-fccd-4d4a-a28a-cb84f88f6c8b --force",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			project, deployment, err := pagesDeploymentArgs(args)
			if err != nil {
			...
			// Rollback changes what visitors see in production, so it confirms
			// like the destructive commands do.
			if !force && !g.DryRun {
				if !confirm(fmt.Sprintf("Roll back Pages project %s to production deployment %s?", project, deployment)) {
					return errors.New("aborted (pass --force to skip confirmation)")
				}
			}
