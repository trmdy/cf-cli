...
// production_branch is required by the API, so an emptied flag must fail
// locally rather than being dropped from the body.
func TestBuildPagesProjectBodyRequiresProductionBranch(t *testing.T) {
	for _, branch := range []string{"", "   "} {
		_, err := buildPagesProjectBody("my-site", branch, "", "", "")
		if err == nil || !strings.Contains(err.Error(), "--production-branch cannot be empty") {
			t.Fatalf("branch %q: expected production branch error, got %v", branch, err)
		}
	}
}

func TestPagesProjectCreateRejectsEmptyProductionBranch(t *testing.T) {
	_, _, err := runPagesCLI(t, "http://example.invalid",
		"pages", "project", "create", "my-site", "--production-branch", "", "--dry-run")
	if err == nil || !strings.Contains(err.Error(), "--production-branch cannot be empty") {
		t.Fatalf("expected production branch error, got %v", err)
	}
}

// The body always carries production_branch, even when the flag is left at its
// default, so the field can never be omitted by a struct tag.
func TestPagesProjectCreateAlwaysSendsProductionBranch(t *testing.T) {
	d := pagesDryRun(t, "pages", "project", "create", "my-site")
	var body map[string]any
	if err := json.Unmarshal(d.Body, &body); err != nil {
		t.Fatal(err)
	}
	if body["production_branch"] != "main" {
		t.Errorf("production_branch = %v, want main", body["production_branch"])
	}
}
...
func TestPagesHelpIncludesExamples(t *testing.T) {
	cases := []struct {
		...
		// The help must describe what the request actually does: create makes a
		// Direct Upload project (no source object is sent), and rollback only
		// works against successful production deployments.
		{[]string{"pages", "project", "create", "--help"}, []string{"cf pages project create my-site", "--production-branch", "--build-command", "Direct Upload", "does not connect a Git repository"}},
		{[]string{"pages", "deployment", "rollback", "--help"}, []string{"cf pages deployment rollback my-site", "--force", "production deployment", "successful production deployment"}},
