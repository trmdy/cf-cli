...
func TestBuildD1CreateBody(t *testing.T) {
	body, err := buildD1CreateBody("app-data", "eu", "weur", "AUTO")
	if err != nil {
		t.Fatal(err)
	}
	d1AssertJSONEqual(t, body, `{"name":"app-data","jurisdiction":"eu","primary_location_hint":"weur","read_replication":{"mode":"auto"}}`)
