...
func newR2Cmd(g *globalOpts) *cobra.Command {
	...
	cmd.AddCommand(newR2BucketCmd(g))
	return cmd
}

func newR2BucketCmd(g *globalOpts) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "bucket",
		Short: "Manage R2 buckets",
		Args:  cobra.NoArgs,
...
func r2BucketName(bucketName string) (string, error) {
	...
	if len(bucketName) < 3 || len(bucketName) > 64 {
		return "", errors.New("bucket name must be between 3 and 64 characters")
	}
	return bucketName, nil
}

func r2LocationHint(location string) (string, error) {
	location = strings.ToLower(strings.TrimSpace(location))
	switch location {
	case "apac", "eeur", "enam", "weur", "wnam", "oc":
		return location, nil
	case "":
		return "", errors.New("--location cannot be empty")
	default:
		return "", errors.New("--location must be one of apac, eeur, enam, weur, wnam, or oc")
	}
}
...
func r2ListBuckets(ctx context.Context, client *api.Client, req api.Request) ([]r2Bucket, error) {
	...
	for page := 1; page <= r2MaxPages; page++ {
		...
		req.Query.Set("cursor", env.ResultInfo.Cursor)
	}
...
func newR2CreateCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Create an R2 bucket.\n\nExamples:\n\n  cf r2 bucket create backups\n  cf r2 bucket create eu-assets --location weur",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("location") {
				location, err = r2LocationHint(location)
				if err != nil {
					return err
				}
