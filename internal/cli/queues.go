...
func newQueuesCmd(g *globalOpts) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Manage Cloudflare Queues. Queue-scoped commands accept a queue name or ID; when given a name, cf looks it up through the API, including during --dry-run.",
	}
	cmd.AddCommand(
		newQueuesListCmd(g),
		newQueuesGetCmd(g),
...
func queuePath(accountID, queueID string) string {
	return queuesPath(accountID) + "/" + url.PathEscape(queueID)
}
...
// resolveQueueID accepts a queue ID or resolves a queue name via the account
// queue list. This intentionally performs the lookup during --dry-run so the
// printed request always contains the API's required queue ID.
func resolveQueueID(ctx context.Context, client *api.Client, accountID, queue string) (string, error) {
	if isQueueID(queue) {
		return queue, nil
	}
	env, err := client.DoAutoPaginate(ctx, api.Request{Method: "GET", Path: queuesPath(accountID)})
	if err != nil {
		return "", fmt.Errorf("look up queue %q: %w", queue, err)
	}
	var queues []struct {
		ID   string `json:"queue_id"`
		Name string `json:"queue_name"`
	...
	if err := json.Unmarshal(env.Result, &queues); err != nil {
		return "", fmt.Errorf("look up queue %q: unexpected response", queue)
	}
	for _, candidate := range queues {
		if candidate.Name == queue {
			return candidate.ID, nil
		}
	}
	return "", fmt.Errorf("queue %q not found in this account; run `cf queues list` to see available queues", queue)
}

// isQueueID reports whether s looks like a Cloudflare queue ID (32 hex chars).
...
func newQueuesGetCmd(g *globalOpts) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			queueID, err := resolveQueueID(cmd.Context(), client, cfg.AccountID, args[0])
			if err != nil {
				return err
			}
			return runQueuesRequest(cmd, g, client, api.Request{Method: "GET", Path: queuePath(cfg.AccountID, queueID)})
		},
	}
}
...
Examples:
  ...
  cf queues update events --delivery-delay 30 --message-retention-period 86400`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "PATCH", Path: queuePath(cfg.AccountID, queueID), Body: body})
		},
	}
...
func newQueuesDeleteCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "DELETE", Path: queuePath(cfg.AccountID, queueID)})
		},
	}
...
Examples:
  ...
  cf queues consumer add events --type http-pull --batch-size 10 --visibility-timeout-ms 30000`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "POST", Path: queuePath(cfg.AccountID, queueID) + "/consumers", Body: body})
		},
	}
...
func newQueuesConsumerRemoveCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := queuePath(cfg.AccountID, queueID) + "/consumers/" + url.PathEscape(args[1])
...
Examples:
  ...
  cf queues message send events '{"event":"created"}' --json --delay-seconds 30`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "POST", Path: queuePath(cfg.AccountID, queueID) + "/messages", Body: body})
		},
	}
	cmd.Flags().BoolVar(&jsonBody, "json", false, "treat body as a JSON value")
...
func buildQueueMessageBody(body string, jsonBody bool, delaySeconds int, includeDelay bool) ([]byte, error) {
	...
	if jsonBody {
		...
		if err := json.Unmarshal([]byte(body), &value); err != nil {
			return nil, errors.New("--json body must be valid JSON")
		}
		payload["body"] = value
...
func newQueuesMessagePullCmd(g *globalOpts) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "POST", Path: queuePath(cfg.AccountID, queueID) + "/messages/pull", Body: body})
		},
	}
...
func newQueuesMessageAckCmd(g *globalOpts) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runQueuesRequest(cmd, g, client, api.Request{Method: "POST", Path: queuePath(cfg.AccountID, queueID) + "/messages/ack", Body: body})
		},
	}
}
