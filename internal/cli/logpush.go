...
func buildLogpushJobBody(cmd *cobra.Command, options logpushJobOptions, create bool) ([]byte, error) {
	...
	if cmd.Flags().Changed("max-upload-bytes") {
		if err := validateLogpushBatchLimit("max-upload-bytes", options.maxUploadBytes, 5_000_000, 1_000_000_000); err != nil {
			return nil, err
		}
		body["max_upload_bytes"] = options.maxUploadBytes
	}
	if cmd.Flags().Changed("max-upload-interval") {
		if err := validateLogpushBatchLimit("max-upload-interval", options.maxUploadInterval, 30, 300); err != nil {
			return nil, err
		}
		body["max_upload_interval_seconds"] = options.maxUploadInterval
	}
	if cmd.Flags().Changed("max-upload-records") {
		if err := validateLogpushBatchLimit("max-upload-records", options.maxUploadRecords, 1_000, 1_000_000); err != nil {
			return nil, err
		}
...
// validateLogpushBatchLimit accepts zero to disable batching, otherwise the
// API's inclusive range for that batch setting.
func validateLogpushBatchLimit(flag string, value, min, max int) error {
	if value == 0 {
		return nil
	}
	if value < min || value > max {
		return fmt.Errorf("--%s must be 0 or between %d and %d", flag, min, max)
	}
	return nil
}
