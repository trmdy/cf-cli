...
func newKVKeyListCmd(g *globalOpts) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("limit") {
				if limit < 10 || limit > 1000 {
					return errors.New("--limit must be between 10 and 1000 (Cloudflare KV keys list range)")
				}
	...
	cmd.Flags().IntVar(&limit, "limit", 0, "maximum keys per request, 10-1000 (disables auto-pagination)")
...
By default the value is written to stdout as raw bytes (not JSON-wrapped).
Pass --output json to render the value as JSON (JSON values pass through;
other values become a JSON string). --query requires --output json.

Examples:

  cf kv key get session:abc --namespace my-app-config
  cf kv key get config.json --namespace 0f2ac74b498b48028cb68387c421e279
  cf kv key get config.json --namespace my-app-config --output json
  cf kv key get config.json --namespace my-app-config --output json --query .foo`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			key := args[0]
			...
			return runKVKeyGet(cmd, g, client, req)
		},
	}
	cmd.Flags().StringVar(&namespace, "namespace", "", "namespace ID or title")
	_ = cmd.MarkFlagRequired("namespace")
...
The value is sent as the request body with Content-Type text/plain.
Use --value @file to read from a file, or --value @- to read from stdin.

Per-key metadata on a single put is out of scope (the API requires
multipart/form-data). To attach metadata, use bulk-put with a JSON file
that includes a "metadata" field on each pair.
...
Examples:
  ...
  cf kv key put session:abc --namespace my-app-config --value token --ttl 3600`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runKVKeyPut(cmd, g, client, req)
		},
	}
	cmd.Flags().StringVar(&namespace, "namespace", "", "namespace ID or title")
...
Use bulk-put (not single-key put) when you need per-key metadata — single
put cannot send multipart metadata through this CLI.
...
// runKVKeyGet uses DoRaw so the stored value is not JSON-quoted by the
// envelope parser. Default output is raw bytes; --output json wraps/renders.
// --query alone must not flip JSON mode — it requires explicit --output json.
func runKVKeyGet(cmd *cobra.Command, g *globalOpts, client *api.Client, req api.Request) error {
	if g.Query != "" && !strings.EqualFold(g.Output, "json") {
		return errors.New("--query requires --output json for kv key get (default output is raw bytes)")
	}
	if g.DryRun {
		dump, err := client.Dump(req)
		if err != nil {
	...
	if !strings.EqualFold(g.Output, "json") {
		_, err := cmd.OutOrStdout().Write(raw.Body)
		return err
	}
...
// runKVKeyPut sends text/plain via DoRaw (errors still surface as *APIError).
// Successful CF responses are JSON envelopes; we render the result field.
func runKVKeyPut(cmd *cobra.Command, g *globalOpts, client *api.Client, req api.Request) error {
	if g.DryRun {
		dump, err := client.Dump(req)
		if err != nil {
			return err
	...
	var env api.Envelope
	if err := json.Unmarshal(raw.Body, &env); err == nil && len(env.Result) > 0 {
		return g.renderResult(cmd, env.Result, output.JSON)
	}
	if json.Valid(raw.Body) {
		return g.renderResult(cmd, raw.Body, output.JSON)
