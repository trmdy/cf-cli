...
func TestParseImagesMetadataObject(t *testing.T) {
	obj, err := parseImagesMetadataObject(`{"album":"home"}`)
	if err != nil {
		t.Fatal(err)
	}
	if obj["album"] != "home" {
	...
	for _, raw := range []string{"null", "[]", `"x"`, "1", "true", "{"} {
		if _, err := parseImagesMetadataObject(raw); err == nil || !strings.Contains(err.Error(), "--metadata must be a JSON object") {
			t.Fatalf("raw %q: expected object error, got %v", raw, err)
		}
	}
}

func TestBuildImagesUploadFormRejectsNullMetadata(t *testing.T) {
	if _, _, _, err := buildImagesUploadForm("", "https://example.com/a.png", "", "null", "", false, false); err == nil || !strings.Contains(err.Error(), "--metadata") {
		t.Fatalf("expected null metadata error, got %v", err)
	}
	if _, _, _, err := buildImagesUploadForm("", "https://example.com/a.png", "", "[]", "", false, false); err == nil || !strings.Contains(err.Error(), "--metadata") {
		t.Fatalf("expected array metadata error, got %v", err)
	}
}

func TestImagesListQueryCreator(t *testing.T) {
	q := imagesListQuery(1, 100, "", false)
	if _, ok := q["creator"]; ok {
		t.Fatalf("unset creator should omit key, got %v", q)
	}
	q = imagesListQuery(2, 50, "", true)
	if _, ok := q["creator"]; !ok {
		t.Fatal("explicit empty creator must set creator=")
	}
	if q.Get("creator") != "" {
		t.Fatalf("creator = %q", q.Get("creator"))
	}
	if q.Get("page") != "2" || q.Get("per_page") != "50" {
		t.Fatalf("q = %v", q)
	}
	q = imagesListQuery(1, 100, "user-1", true)
	if q.Get("creator") != "user-1" {
		t.Fatalf("creator = %q", q.Get("creator"))
	}
}

func TestRequireImagesAccountID(t *testing.T) {
	if _, err := requireImagesAccountID(""); err == nil || !strings.Contains(err.Error(), "account ID") {
		t.Fatalf("expected account error, got %v", err)
	}
	id, err := requireImagesAccountID(imagesTestAccountID)
	if err != nil || id != imagesTestAccountID {
...
func TestImagesListDryRunEmptyCreator(t *testing.T) {
	stdout, _, err := runImagesCLI(t, "http://example.invalid",
		"images", "list",
		"--creator", "",
		"--dry-run",
	...
	// Explicit empty creator must appear as creator= in the query string.
	if !strings.Contains(dump.URL, "creator=") {
		t.Fatalf("expected creator= in url, got %s", dump.URL)
	}
	if strings.Contains(dump.URL, "creator=user") {
		t.Fatalf("unexpected creator value: %s", dump.URL)
	}
}

func TestImagesListDryRunOmitsUnsetCreator(t *testing.T) {
	stdout, _, err := runImagesCLI(t, "http://example.invalid",
		"images", "list",
		"--dry-run",
	)
	...
	if strings.Contains(dump.URL, "creator=") {
		t.Fatalf("unset creator should be omitted, got %s", dump.URL)
	}
}
...
func TestImagesListPaginatesExplicitEmptyCreator(t *testing.T) {
	var creators []string
	var creatorPresent []bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, ok := r.URL.Query()["creator"]
		creatorPresent = append(creatorPresent, ok)
		creators = append(creators, r.URL.Query().Get("creator"))
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Query().Get("page") {
		case "1":
			var b strings.Builder
			...
			_, _ = w.Write([]byte(b.String()))
		case "2":
			_, _ = w.Write([]byte(`{"success":true,"result":{"images":[{"id":"e2-0"}]}}`))
		default:
			t.Fatalf("unexpected page %s", r.URL.Query().Get("page"))
		}
	}))
	...
	_, _, err := runImagesCLI(t, srv.URL, "images", "list", "--creator", "", "--output", "json")
	if err != nil {
		t.Fatal(err)
	}
	if len(creators) != 2 {
		t.Fatalf("requests = %d", len(creators))
	}
	for i, present := range creatorPresent {
		if !present {
			t.Fatalf("page %d missing creator query key", i+1)
		}
		if creators[i] != "" {
			t.Fatalf("page %d creator = %q", i+1, creators[i])
		}
	}
}
...
func TestImagesUploadDryRunURL(t *testing.T) {
	...
	// Dump quotes non-JSON bodies as a JSON string containing the multipart payload.
	bodyStr, _ := dump.Body.(string)
	if bodyStr == "" {
		// sometimes rendered as raw quoted via json.RawMessage
		raw, _ := json.Marshal(dump.Body)
		bodyStr = string(raw)
	}
	for _, want := range []string{"https://example.com/logo.png", "brand-logo", "requireSignedURLs"} {
...
func TestBuildImagesMultipart(t *testing.T) {
	body, ct, err := buildImagesMultipart(map[string]string{
	...
	if !bytes.Contains(body, []byte("https://example.com/a.png")) || !bytes.Contains(body, []byte("name=\"id\"")) {
...
func TestImagesUploadRejectsNullMetadataCLI(t *testing.T) {
	_, _, err := runImagesCLI(t, "http://example.invalid",
		"images", "upload",
		"--url", "https://example.com/logo.png",
		"--metadata", "null",
